...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 12 04:28:37 2023
...
for i in range(7):  # Predict 7 days into the future
    ...
    # Shift the window of current_batch
    current_batch = np.roll(current_batch, -1, axis=1)
    
    # Update the target feature in the last timestep with the predicted value
    current_batch[0, -1, target_feature_index] = current_pred[-1]  # Update only the target feature
    
    # Optionally: Update the rest of the features for the new day
    # Here we carry over the last known values (repeat the last day's non-target features)
...
# Assuming the following values for the calculation:
...
# Volatility of the stock (assumed)
sigma = 0
...
for quantile in quantiles:
    ...
    # Initialize an empty list to hold the option prices for each predicted stock price
    option_prices = []
    
    # Loop through each predicted stock price and calculate the option price
    ...
    plt.axhline(y=0, color='black', linestyle='-')  # Add a line at y=0 for reference
    plt.show()